// サイトに出す文言はすべてここに集約する。
// 文面の正本は ~/Projects/案件応募/ の各mdファイル。修正はまずそちらを直すこと。

/**
 * 問い合わせ先の設定。
 * 未設定（PLACEHOLDER のまま）のチャンネルは、サイト上に表示されない。
 * → LINEだけ用意できた段階で公開でき、フォームは後から足せる。
 * 両方 PLACEHOLDER のままだと問い合わせ手段がゼロになるので、
 *   npm run check で公開前に検出する。
 */

/** LINE公式アカウントの友だち追加URL（https://lin.ee/xxxxx） */
export const LINE_URL = "https://lin.ee/PLACEHOLDER";
/** Formspree のフォームID（https://formspree.io/f/xxxxxxxx の xxxxxxxx 部分） */
export const FORMSPREE_ID = "xbgjargn";

export const hasLine = !LINE_URL.includes("PLACEHOLDER");
export const hasForm = !FORMSPREE_ID.includes("PLACEHOLDER");

export const hero = {
  headline: "毎日くり返している手作業を\nやらなくていい形にします。",
  sub: "宮崎県で畜産の現場に18年、今も現場にいます。\nそのうえで、LINE・Slack・Googleと業務をつなぐ仕組みを自分で作ります。",
  badge: "自作の繁殖管理アプリは、県内の畜産農家2戸で稼働中",
};

export type Capability = {
  title: string;
  body: string;
  anchor: string;
  lead?: boolean;
};

export const capabilities: Capability[] = [
  {
    title: "現場で使う記録・管理の仕組み",
    body: "事務所のパソコンではなく、作業をしているその場でスマホから記録・確認できる形にします。後回しにならないので、記録が続きます。",
    anchor: "work-a1",
    lead: true,
  },
  {
    title: "問い合わせ対応の自動化",
    body: "LINEやWebサイトに届く「よくある質問」に、24時間自動で答える窓口を作ります。判断が必要なものだけ、手元に通知が届きます。",
    anchor: "work-a2",
  },
  {
    title: "社内の資料を、聞かなくても分かるように",
    body: "マニュアル、規程、過去の通知をAIに読ませて、質問すれば答えが返る仕組みを作ります。普段使っているSlackの中で完結させることもできます。",
    anchor: "work-b1",
  },
  {
    title: "毎回くり返している作業の自動化",
    body: "毎月の転記、商品説明文の作成、複数の場所への連絡、売上の集計。手順が決まっている作業は、自動で終わらせて、人は確認だけにできます。",
    anchor: "work-a3",
  },
];

export type Work = {
  id: string;
  title: string;
  kicker?: string;
  problem: string;
  built: string;
  result: string;
  tech: string;
  demo?: { label: string; url: string };
  featured?: boolean;
};

/** 第1部：実際に動いているもの */
export const liveWorks: Work[] = [
  {
    id: "work-a1",
    title: "繁殖牛管理アプリ（スマホ）",
    kicker: "県内の畜産農家2戸で稼働中",
    featured: true,
    problem:
      "牛の繁殖管理は、種付け日から逆算して発情と分娩の予定を追い続ける仕事です。これをパソコンや紙の台帳でやると、牛舎にいる間は確認できません。手が空いてから事務所で確認することになり、発情の見逃しや分娩事故につながります。牛の発情周期は約21日。一度見逃せば、次の機会は3週間後です。",
    built:
      "牛舎で、スマホを開いたその場で確認・記録できる繁殖管理アプリです。記録を入れると、次に手を打つべき牛の順に自動で並び替わり、待機中の牛が先頭に来ます。開いた瞬間に「今日どれを見るか」が決まります。過去の記録も後から修正できます。アプリのインストールは不要です。",
    result:
      "宮崎県内の繁殖農家2戸に導入し、現在も使われています。発情の見逃しと分娩事故を減らすことが目的で、パソコン管理からの時間短縮も伴いますが、本当の効果は経営面にあります。発情を1回見逃せば分娩は3週間遅れ、それがそのまま1頭あたりの年間収益に響くためです。",
    tech: "Next.js / TypeScript / Supabase / Vercel（PWA）",
  },
  {
    id: "work-a2",
    title: "LINEで送るだけの予定登録",
    problem:
      "予定をカレンダーに入れるのに、アプリを開いて日時・場所・分類を打ち込む必要がありました。1件あたり5分。移動中や作業中は後回しになり、そのまま忘れます。入れ忘れた予定は当日まで気づきません。",
    built:
      "LINEに文章を1行送るだけで、AIが日時・場所・内容を読み取り、Googleカレンダーに登録する仕組みです。予定の種類ごとに色分けまで自動でつきます。さらに毎朝、その日の予定と今週の予定がLINEに自動で届きます。新しいアプリを覚える必要がなく、普段使っているLINEの中で完結します。",
    result:
      "1件5分かかっていた入力が、LINEを1行送る十数秒になりました。毎朝の自動通知によって、予定の見落としがなくなりました。",
    tech: "LINE Messaging API / Python / Google Calendar API / Render / GitHub Actions",
  },
  {
    id: "work-a3",
    title: "会計データの転記を自動化",
    problem:
      "会計ソフトから落としたCSVを、毎月スプレッドシートに手で貼り直して整形していました。1回30分。月に1度きりの作業ですが、毎回同じ手順で、毎回同じだけ時間がかかります。年間では6時間です。",
    built:
      "前月分のCSVを渡すと、必要な列だけを抜き出して整形し、スプレッドシートの所定の位置まで自動で書き込む仕組みです。",
    result: "月30分の転記作業が、実行して待つだけの数分に。年間で約6時間が空きました。",
    tech: "Python / Google Sheets API / マネーフォワードクラウド",
  },
  {
    id: "work-a4",
    title: "副業時間トラッカー",
    problem:
      "副業に使った時間を記録しておらず、月にどれだけ使ったか把握できていませんでした。市販のツールは項目が多くて続きません。",
    built:
      "ブラウザで開くだけの記録画面です。開始と終了を押すだけで、暦月区切りの累計が出ます。インストールもログインも不要です。",
    result: "記録が続くようになり、月ごとの実績が数字で残るようになりました。",
    tech: "HTML / JavaScript",
  },
];

/** 第2部：講座の課題として制作した模擬案件 */
export const caseWorks: Work[] = [
  {
    id: "work-b1",
    title: "社内資料の検索AI",
    problem:
      "農業法人・組合の事務所で、飼養管理マニュアル、防疫の通知文、過去の指導記録が紙とPDFに散らばっている。「あの通知どこだっけ」を探すのに毎回10〜15分。分からなければ詳しい人に聞きに行くので、聞かれた側の手も止まる。",
    built:
      "手持ちの資料をAIに読み込ませ、日本語で質問すると答えが返る検索の仕組みです。回答には「どの資料の何ページか」が併記されるので、原本の確認まで一直線です。",
    result:
      "資料探しが10〜15分から30秒に。ベテランへの「これ何だっけ」という質問も減ります。（1日3回 × 12分 ＝ 約36分/日）",
    tech: "Python / ベクトル検索 / Claude API",
  },
  {
    id: "work-b2",
    title: "Slackの中で答える社内AI",
    problem:
      "従業員30名ほどの会社で、経費精算のやり方、有給の申請手順、就業規則の細かい規定を、社員が総務に都度聞きに来る。総務側は1日に何度も同じ説明をしている。",
    built:
      "普段使っているSlackで質問すると、社内規程を読んだAIがその場で答えます。検索専用の画面を開くのではなく、いつものSlackの中で完結するので、新しいツールを覚える必要がありません。部署ごとに見せる資料を分けることもできます。",
    result:
      "総務への定型質問が大幅に減ります。社員側も、聞きに行く手間と気まずさがなくなります。（1日5件 × 5分 ＝ 約25分/日。聞きに行く側の移動時間は別）",
    tech: "Python / Slack API / ベクトル検索 / Claude API",
    demo: { label: "実際に動くものを見る", url: "https://case7-rag-slack.vercel.app" },
  },
  {
    id: "work-b3",
    title: "Webサイトの問い合わせ自動対応",
    problem:
      "小規模な宿泊施設・体験施設に、Webサイトから同じ質問が届く。チェックイン時間、駐車場、アレルギー対応、雨天時の扱い。夜間や早朝にも来るが、返信できるのは翌営業日。その間に他をあたられる。",
    built:
      "Webサイトに設置する問い合わせAIです。施設のFAQを読み込ませ、24時間即答します。予約変更など判断が要るものは、内容を要約して担当者にメールで転送します。",
    result:
      "夜間・休業日の問い合わせに、待ち時間ゼロで返せます。担当者に回るのは全体の2〜3割だけになります。（問い合わせの7〜8割が定型質問という前提）",
    tech: "Next.js / Claude API / Vercel",
  },
  {
    id: "work-b4",
    title: "売上の見える化ダッシュボード",
    problem:
      "複数の販路（フリマアプリ・自社EC・イベント出店）で売っている個人事業主が、月末に各サイトからCSVを落として手作業で集計している。毎月2〜3時間。しかも集計が終わるのは月が変わってから。動きが遅れる。",
    built:
      "各販路のデータを取り込み、売上・売れ筋・在庫回転を1画面にまとめます。月末を待たずに、いつ見ても最新の数字が出ています。",
    result:
      "月2〜3時間の集計作業がゼロに。判断が「翌月」から「その日」に早まります。（月3販路 × 各50分 ＝ 約2.5時間/月）",
    tech: "Next.js / TypeScript / Supabase / Vercel",
  },
  {
    id: "work-b5",
    title: "商品説明文とブログ記事の自動生成",
    problem:
      "ネットショップの運営者が、商品を出すたびに説明文と紹介記事を手書きしている。1商品あたり20〜30分。週に10点出すと、それだけで4〜5時間が消える。",
    built:
      "スプレッドシートに商品名と特徴を入力すると、AIが商品説明文とブログ記事を自動で生成し、下書きとして保存します。人は確認して直すだけです。",
    result:
      "1商品30分の執筆が、確認と手直しの5分に。週10点で4時間以上が空きます。（10点 × 25分短縮 ＝ 約4時間/週）",
    tech: "Google Apps Script / Claude API / Googleスプレッドシート",
  },
];

export const profile = {
  name: "押田 竜太",
  reading: "おしだ りゅうた",
  brand: "RyuWorks",
  area: "宮崎県",
  body: [
    "畜産の現場で18年、今も働いています。",
    "現場の仕事は、パソコンの前に座ってするものではありません。だから記録は後回しになり、後回しになったものが見落としや事故につながる。どこで手が止まるのか、何が続かないのかを、やってきた側として知っています。",
    "その延長で作ったのが繁殖管理アプリで、いまは県内の畜産農家2戸に使っていただいています。使っている方から要望をもらって直しながら続けています。",
    "ほかに、社内資料の検索、問い合わせの自動応答、売上の集計といった仕組みを作っています。",
  ],
  closing: [
    "「業務を知らない開発者」でも「作れないコンサル」でもありません。現場で面倒だと思っていたことを、そのまま形にします。",
    "まずは話を聞くだけでも構いません。",
  ],
  availability: "平日の早朝と夜、土日を中心に対応しています。",
};
